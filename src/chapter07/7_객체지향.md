...
- private : 자신의 클래스 에서만 접근할 수 있는 권한을 가짐 (우리집 화장실)
- default(package) : 같은 패키지에서 존재하는 클래스내에서만 접근할 수 있는 권한을 가짐 (아파트 1층 화장실)
- protected : 상속관계를 가진 클래스에서만 접근할 수 있는 권한을 가짐 (부모님집 화장실(상속관계))
- public : 모든 클래스에서 접근할 수 있는 권한을 가짐 (공원 공용 화장실)
